...
var playersCardHeight = 125, playersCardWidth = 86;
var prevCardScale = 0.8;
var prevCardHeight = prevCardScale*playersCardHeight, prevCardWidth = prevCardScale*playersCardWidth;
var oppCardScale = 0.8;
var oppCardHeight = oppCardScale*playersCardHeight, oppCardWidth = oppCardScale*playersCardWidth;

var oppCardDeltaX = 25;
var prevCardDeltaX = oppCardDeltaX, prevCardDeltaY = 35;
var playerCardDeltaX = (1.0/oppCardScale)*oppCardDeltaX;
var playerSelectedDeltaY = 20;

var playerCardInitialX = 480, playerCardInitialY = 600;
var prevCardInitialX = 680, prevCardInitialY = 280;
var oppInitialX = [100, 529, 1300], oppInitialY = [200, 50, 200];
var oppCardDirections = [[0,1],[1,0],[0,1]];
var oppCardRot = [-Math.PI/2, 0, Math.PI/2];

var whosGoCirclePos = [[720,530],[250,400],[720,200],[1200,400]]
...
function drawPlayersHand() {
    ...
    for(var i=0; i<playersHand.length; i++) {
        ...
        card.innerHTML = ("<img src='" + staticURL + "cardImages/" + String(playersHand[i]) + ".png' width='" + String(playersCardWidth) + "' height='" + String(playersCardHeight) + "'/>");
...
function drawOpponentsHands() {
    ...
    for(var opp=0; opp<3; opp++) {
        ...
        for(var i=0; i<nOppCards[opp]; i++) {
            ...
            ctx.translate(-1*(ox + oppCardWidth/2), -1*(oy+oppCardHeight/2));
...
function drawPreviousHands() {
    ...
    for(var j=0; j<3; j++) {
        for(var i=0; i<prevHands[j].length; i++) {
            ...
            card.innerHTML = ("<img src='" + staticURL + "cardImages/"+String(prevHands[j][i])+".png' width='"+String(prevCardWidth)+"' height='"+String(prevCardHeight)+"'/>");
...
function updateScores() {
    ...
    for(var i=0; i<4; i++) {
        ...
        playerText.innerHTML = "P" + String(i+1) + ": " + String(sessionScores[i]);
    }
