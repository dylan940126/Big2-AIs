...
function isFlush(hand) {
	if(hand.length != 5) return 0;
	if(hand[0] % 4 == hand[1] % 4 && hand[0] % 4 == hand[2] % 4 && hand[0] % 4 == hand[3] % 4 && hand[0] % 4 == hand[4] % 4 && !isStraightFlush(hand)) {
		return 1;
	} else {
		return 0;
	}
...
function compareHands(hand1, hand2) { //returns 1 if hand1 beats hand2, 0 if hand2 beats hand1.
	...
	if(hand1.length==1) {
		...
		if(isStraight(hand1) && (isFlush(hand2) || isFullHouse(hand2) || isStraightFlush(hand2))) {
			...
			return 0;
		} else if((isFullHouse(hand1) || isStraightFlush(hand1)) && (isFlush(hand2) || isStraight(hand2))) {
			return 1;
