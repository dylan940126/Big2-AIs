...
def run_game(model_state_dict) -> List[float]:
    ...
    agents: List[Agent] = [
        CNNAgent(model=cnn_model, train=False),  # Don't train during evaluation
        RandomAgent(),
        RandomAgent(),
        RandomAgent()
    ]
...
def evaluate_model(model, num_games=100):
    ...
    with multiprocessing.Pool(processes=cpu_count) as pool:
        results = list(tqdm(pool.imap(run_game, repeat(model_state_dict, num_games)), 
                           total=num_games, desc="評估中"))
...
if __name__ == "__main__":
    num_games = 100
    training_games = 10  # Number of games to train before evaluation
    
    cnn_model = Big2CNN()
    # Load existing model if available
    try:
        cnn_model.load_state_dict(torch.load("cnn_agent2.pt"))
        print("載入現有模型...")
    ...
    while True:
        ...
        training_wins = np.sum(training_rewards == np.max(training_rewards, axis=1, keepdims=True), axis=0)
        training_win_rates = training_wins / training_games * 100
        
        print(f"訓練結果 - CNN獲勝率: {training_win_rates[0]:.1f}%, 平均獎勵: {avg_training_rewards[0]:.2f}")
        ...
        if episode % 5 == 0:
            torch.save(cnn_model.state_dict(), f"cnn_agent_episode_{episode}.pt")
            print(f"模型已保存至 cnn_agent_episode_{episode}.pt")
        
        # Save best model if CNN win rate is improving
        if win_rates[0] > 20:  # If CNN wins more than 20%
            torch.save(cnn_model.state_dict(), "cnn_agent_best.pt")
            print("🎉 新的最佳模型已保存！")
        torch.save(cnn_model.state_dict(), "cnn_agent.pt")
